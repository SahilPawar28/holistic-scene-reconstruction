...
| **Placement / assembly solver** | **not yet implemented** |
| Scene composition → single .glb | **not yet implemented** |
| Baseline comparison writeup | not started |

Tier 1 works today. Tier 2 has every stage built *except* the one that
decides where each generated object goes — `pipeline/assembly.py` and
`pipeline/scene_compose.py` do not exist yet. Until they do, the `/scene`
endpoint runs all the earlier stages (so their diagnostics are real and
visible in the UI) and then falls back to returning the Tier 1 mesh.
...
pipeline/
  ...
  assembly.py       ← placement solver (not yet written)
  scene_compose.py  ← shell + placed objects → one .glb (not yet written)
...
Four decisions in here are load-bearing, and each one came from something
that actually broke.
...
Both run in seconds on CPU, and both check against known values — that the
