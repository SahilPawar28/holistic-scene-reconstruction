...
def filter_instances(
    ...
    Applied in this order, cheapest and most decisive test first:
      ...
      3. containment / IoU    — parts of an already-kept object, or dupes
      4. count cap            — keep the largest N

    Order matters for step 3: instances are sorted largest-first, so a whole
    ...
    for inst in sorted(instances, key=lambda i: i.area, reverse=True):
        ...
        if frac > params.max_area_fraction or frac < params.min_area_fraction:
            continue
        kept.append(inst)
...
class SAM2Segmenter:
    ...
    def segment(
        self, image: Image.Image, depth: np.ndarray | None = None
    ) -> list[Instance]:
